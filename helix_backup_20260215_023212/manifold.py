...
# Import the lens system
from .substrate import NaturalLens, ColorLens, SoundLens, ValueLens
...
class SurfacePoint:
    ...
    # -------------------------------------------------------------------------
    # NATURAL LENS - Color, Sound, Value derived from geometry
    # -------------------------------------------------------------------------
    
    @cached_property
    def lens(self) -> NaturalLens:
        """
        Natural lens: Color, Sound, and Value derived from actual geometry.
        
        - Color: From azimuth angle → visible spectrum wavelength
        - Sound: From magnitude → resonating frequency
        - Value: Raw geometric properties
        """
        return NaturalLens.from_geometry(self.x, self.y, self.z)
    
    @cached_property
    def color(self) -> ColorLens:
        """Color derived from azimuth angle"""
        return self.lens.color
    
    @cached_property
    def sound(self) -> SoundLens:
        """Sound derived from magnitude"""
        return self.lens.sound
    
    @cached_property
    def value_lens(self) -> ValueLens:
        """Raw geometric value properties"""
        return self.lens.value
