...
class AudioFrame:
    ...
    def deserialize(cls, data: bytes) -> 'AudioFrame':
        ...
        if len(data) < cls.HEADER_SIZE:
            raise ValueError(f"AudioFrame requires at least {cls.HEADER_SIZE} bytes, got {len(data)}")
        
        (spiral, level, seq, ts, payload_len, 
         samples, channels, bits) = struct.unpack(cls.HEADER_FORMAT, data[:cls.HEADER_SIZE])
        
        # Validate payload_len (max 10MB)
        ...
        if len(data) < cls.HEADER_SIZE + payload_len:
            raise ValueError(f"Truncated audio frame: need {cls.HEADER_SIZE + payload_len} bytes, got {len(data)}")
        
        # Validate audio parameters
        if channels > 32:
            raise ValueError(f"Invalid channel count: {channels}")
        if bits not in (8, 16, 24, 32):
            raise ValueError(f"Invalid bits per sample: {bits}")
...
class SyncSignal:
    ...
    def deserialize(cls, data: bytes) -> 'SyncSignal':
        ...
        # Minimum size: 1 (count) + 16 (header) = 17 bytes
        if len(data) < 17:
            raise ValueError(f"SyncSignal requires at least 17 bytes, got {len(data)}")
        
        count = data[0]
        
        # Limit count to prevent memory exhaustion
        if count > 255:
            raise ValueError(f"Too many latency entries: {count}")
        
        # Check we have enough data for all latency pairs
        required = 17 + (count * 4)
        if len(data) < required:
            raise ValueError(f"Truncated SyncSignal: need {required} bytes, got {len(data)}")
